...
interface GoalItemProps {
  ...
  updateGoal: (goal_id: string, description: string) => Promise<void>;
  toggleGoalCompletion: (goal_id: string) => Promise<void>;
}
...
// i need the  { } in the paramter object or else error
  ...
  const { editedText, isEditing, setEditedText, setIsEditing, handleSave, handleInputChange, handleKeyPress } = useEditText();  // vars from hook directly

  // isEditing and editedText  is local to each GoalItem component instance because each goalitem has its own useEditText state, setting one goal_id inside one GoalItem doesn’t affect other instances. That means if you click on another goal’s edit button, the previous goal remains editable because it’s not being updated globally.
          ...
          <svg
            ...
            onClick={() => {
              ...
              if (isEditing === goal.goal_id) {
                ...
                // if its other id... thats not possible since each instance of GoalItem.tsx has its own state
